...
export class CreateUsuarioDto {
   @ApiProperty({ description: 'El correo electrónico del usuario', example: 'usuario19@viu.es' })
   @IsEmail({}, { message: 'El valor del email debe ser un email válido' })
   @IsNotEmpty({ message: "La propiedad 'email' es requerida" })
   readonly email: string;

   @ApiProperty({ description: 'El nombre del usuario', example: 'Javier Lorenzo' })
   @IsString({ message: 'El nombre debe ser una cadena de texto' })
   @MinLength(1, { message: 'El nombre no puede estar vacío' })
   @IsNotEmpty({ message: "La propiedad 'nombre' es requerida" })
   readonly nombre: string;

   @ApiProperty({ description: 'El primer apellido del usuario', example: 'Alonso' })
   ...
   @ApiProperty({ description: 'El segundo apellido del usuario', required: false, example: 'Moirón' })
   @IsOptional()
