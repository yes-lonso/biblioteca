...
export class ResponsePrestamoDto {
   ...
   @IsOptional()
   @Expose()
   @Transform(({ value }) => (value ? formatToSpanish(value) : undefined))
   ...
   /**
    * Estado del préstamo. Se expone en la respuesta.
    */
   @ApiProperty({
      description: 'Estado del préstamo',
      example: 'devuelto | en préstamo',
   })
   @Expose()
   @Transform(({ obj }) =>
      obj.fechaDevolucionReal ? 'Libro ya devuelto' : 'Libro en préstamo',
   )
   estado: string;
}
